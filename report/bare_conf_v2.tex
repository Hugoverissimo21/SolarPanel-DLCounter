...
\begin{abstract}
This project was developed with the Lacuna Solar Survey Challenge in mind, to create a model capable of identifying photovoltaic and thermal solar panels from aerial imagery (satellite or drone). The challenge is of public interest, to learn about solar panel usage, and support in planning infrastructure in Madagascar. The dataset presented issues such as inconsistent masks and a strong class imbalance, which required manual curation. Three types of models were developed: a hybrid approach, of image-based regression combined with metadata based estimation, object detection, and instance segmentation, where the former was the best performing model. The benchmark revealed that there was still a margin for progression, that would require finer curation of the dataset and computational power.
%PERFORMANCE NO Q??? Using a curated dataset from the Lacuna Solar Survey Challenge, the performance of different models are explored, including object detection and image-based regression architectures. Despite challenges such as class imbalance and inconsistent annotations, the hybrid regression model achieved the best performance. The results highlight the impact of data quality and model selection in remote sensing applications for energy infrastructure assessment. (from GPT, don't use)
